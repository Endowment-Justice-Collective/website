...
export default function News(){return <InnerPage eyebrow="News & coverage" title={<>News and reporting</>} intro="Selected coverage of Yale’s endowment, investment policy, and related organizing." tone="cream"><section className="content-section"><div className="news-grid">{stories.map(s=><a className="news-card" key={s.title} href={s.href} target="_blank" rel="noreferrer"><time>{s.date}</time><h2>{s.title}</h2><p>{s.copy}</p><span>Read story</span></a>)}</div></section></InnerPage>}
